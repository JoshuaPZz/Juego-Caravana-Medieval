...
public class CiudadProductoEditController {
    ...
    public ModelAndView getProductosCiudad(@PathVariable("id") Long id) {
        ...
        List<CiudadProductoDTO> ciudadProductoDTOs = ciudadService.getCiudadProducto(id).get();
        List<Long> productosId = ciudadProductoDTOs.stream().map(CiudadProductoDTO :: getIdProducto).toList();
        // List<ProductoDTO> productosCiudad = productoService.listaIdsToProducto(productosId);
        List<ProductoDTO> productos = productoService.getAllProductos();
        ModelAndView modelAndView = new ModelAndView("ciudad-productos-edit");
        modelAndView.addObject("ciudad", ciudad);
        modelAndView.addObject("productos", productos);
        modelAndView.addObject("productosId", productosId);
        return modelAndView;
    }
    @PostMapping("/save")
    public RedirectView saveProductosCiudad(@ModelAttribute CiudadProductoDTO ciudadProductoDTO) {
         ...
         */
        return new RedirectView("/ciudades/edit/" + ciudadProductoDTO.getId());
    }
}
